...
static elem_t *PoisonAllocStack(int start, int max)
{
    #ifdef CANARY_PROT
    ...
    elem_t* buff =(elem_t*)calloc(size_calloc, sizeof(elem_t));
    ...
    else 
    {
        ...
        if(buff == NULL)
        {
            ...
            *buff = DATA_LEFT_CANARY;
            ...
            stack->data++;
    #else 
        elem_t *buff = (elem_t*)realloc(stack->data, stack->capacity * sizeof(elem_t));
        if(buff == NULL)
        {
            printf("Cannot allocate memory\n");
            return MEM_ALLOC_FAIL; 
...
int StackPush(struct Stack *stack, elem_t value) 
{
    Stack_OK(stack);
...
elem_t StackTop(struct Stack *stack, int *error_code)
{
    Stack_OK(stack);

    if(stack->size < 1 && (stack->data[stack->size] == NULL))
    {
...
elem_t StackPop(struct Stack *stack, int *error_code)
{
    ...
    stack->size--; 
    elem_t temp = stack->data[stack->size];
